...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "itorders-application-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name itorders-application-service apps/itorders-application-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app itorders-application-service:1.0-SNAPSHOT
		call oc expose svc/itorders-application-service
	) else (
		echo "Launching the application locally..."
		cd itorders-application-service
		cd target
