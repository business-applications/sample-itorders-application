...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "itorders-application-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name itorders-application-service apps/itorders-application-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."

    oc new-app itorders-application-service:1.0-SNAPSHOT
    oc expose svc/itorders-application-service
else

	echo "Launching the application locally..."
	pattern="itorders-application-service"
	files=( $pattern )
	cd ${files[0]}
